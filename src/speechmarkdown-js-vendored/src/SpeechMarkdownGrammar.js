...
function speechMarkdownGrammar(myna) {
    ...
    var g = new (function () {
        ...
        this.textModifierKey = m.keywords('emphasis', 'address', 'number', 'cardinal', 'characters', 'chars', 'digits', 'drc', 'expletive', 'bleep', 'fraction', 'interjection', 'ordinal', 'telephone', 'phone', 'unit', 'time', 'date', 'whisper', 'ipa', 'sub', 'vol', 'volume', 'rate', 'pitch', 'timbre', 'lang', 'voice', 'excited', 'disappointed').ast;
        ...
        var percentChange = ['+', m.hyphen, m.digit, '%'];
        this.textModifierText = m.choice.apply(m, __spreadArrays([m.digit,
            m.letter,
            m.hyphen,
            m.space], ipaChars, percentChange)).oneOrMore.ast;
